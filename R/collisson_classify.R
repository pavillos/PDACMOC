#' @title Collisson classifier
#' 
#' @author Villoslada-Blanco, Pablo
#' 
#' @description
#' Classification of tumor according to Collisson et al., 2011
#' 
#' @import reticulate
#'
...
#'
#' @return results_collisson: classification according to Collisson
#' 
#' @example ./inst/examples/example.R

collisson.classify <- function(new_samples) {
  
  # Get Collisson genes and extract common genes
  file1 <- system.file('training_data', 'all_Collisson.csv', package = 'PDACMOC')
  ...
  message('Collisson: the expression of ', genes_imputed, ' genes will be imputed')
  ...
  file2 <- system.file('models', 'Collisson.pkl', package = 'PDACMOC')
  ...
  results_collisson <- data.frame('Predicted.subtype' = factor(labels, levels = c('Classical', 'QM', 'Exocrine-like')),
                                  'Probability' = probability,
                                  row.names = rownames(data2ML))
  rm(data2ML, labels, probability)
 
  return(results_collisson)
