#' @title PDAConsensus classifier of stroma
#' 
#' @author Villoslada-Blanco, Pablo
#' 
#' @description
#' Classification of stroma according to PDAConsensus classifier
#' 
#' @import reticulate
#'
...
#'
#' @return results_stroma_consensus: classification according to PDAConsensus of stroma
#' samples
#' 
#' @example ./inst/examples/example.R

stroma.PDAConsensus.classify <- function(vm_S) {
  
  # Get Consensus stromal genes and extract common genes
  file1 <- system.file('training_data', 'all_Consensus_stroma.csv', package = 'PDACMOC')
  ...
  message('Stroma PDAConsensus: the expression of ', genes_imputed, ' genes will be imputed')
  ...
  file2 <- system.file('models', 'Consensus_stroma.pkl', package = 'PDACMOC')
  ...
  rm(sample, max_value)
  results_stroma_consensus <- data.frame('Predicted.subtype' = factor(labels, levels = c('Consensus_class_1', 'Consensus_class_2')),
                                         'Probability' = probability,
                                         'ActivatedECMScore' = probabilities[,2],
