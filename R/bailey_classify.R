#' @title Bailey classifier
#' 
#' @author Villoslada-Blanco, Pablo
#' 
#' @description
#' Classification of tumor according to Bailey et al., 2016
#' 
#' @import reticulate
#'
...
#'
#' @return results_bailey: classification according to Bailey
#' 
#' @example ./inst/examples/example.R

bailey.classify <- function(new_samples) {
  
  # Get Bailey genes and extract common genes
  file1 <- system.file('training_data', 'all_Bailey.csv', package = 'PDACMOC')
  ...
  message('Bailey: the expression of ', genes_imputed, ' genes will be imputed')
  ...
  file2 <- system.file('models', 'Bailey.pkl', package = 'PDACMOC')
  ...
  results_bailey <- data.frame('Predicted.subtype' = factor(labels, levels = c('Squamous', 'Progenitor', 'Immunogenic', 'ADEX')),
                               'Probability' = probability,
                               row.names = rownames(data2ML))
  rm(data2ML, labels, probability)
  
  return(results_bailey)
