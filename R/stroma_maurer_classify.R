#' @title Maurer classifier of stroma
#' 
#' @author Villoslada-Blanco, Pablo
#' 
#' @description
#' Classification of stroma according to Maurer et al., 2019
#' 
#' @import reticulate
#'
...
#'
#' @return results_stroma_maurer: classification according to Maurer of stroma
#' samples
#' 
#' @example ./inst/examples/example.R

stroma.maurer.classify <- function(vm_S) {
  
  # Get Maurer stromal genes and extract common genes
  file1 <- system.file('training_data', 'all_Maurer.csv', package = 'PDACMOC')
  ...
  file2 <- system.file('models', 'Maurer.pkl', package = 'PDACMOC')
  ...
  results_stroma_maurer <- data.frame('Predicted.subtype' = factor(labels, levels = c('ECM-rich', 'Immune-rich')),
                                      'Probability' = probability,
                                      row.names = rownames(vmdata2ML))
  rm(vmdata2ML, labels, probability)
  
  return(results_stroma_maurer)
