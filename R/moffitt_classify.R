#' @title Moffitt classifier
#' 
#' @author Villoslada-Blanco, Pablo
#' 
#' @description
#' Classification of tumor according to Moffitt et al., 2015
#' 
#' @import reticulate
#'
...
#'
#' @return results_moffitt: classification according to Moffitt
#' 
#' @example ./inst/examples/example.R

moffitt.classify <- function(new_samples) {
  
  # Get Moffitt genes and extract common genes
  file1 <- system.file('training_data', 'all_Moffitt.csv', package = 'PDACMOC')
  ...
  message('Moffitt: the expression of ', genes_imputed, ' genes will be imputed')
  ...
  file2 <- system.file('models', 'Moffitt.pkl', package = 'PDACMOC')
  ...
  results_moffitt <- data.frame('Predicted.subtype' = factor(labels, levels = c('Classical', 'Basal-like')),
                                'Probability' = probability,
                                row.names = rownames(data2ML))
  rm(data2ML, labels, probability)
  
  return(results_moffitt)
